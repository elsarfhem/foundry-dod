...
export async function addCardsToPile(sheet) {
  ...
  if (isEmptyCardData(data)) return ui.notifications.warn('There are no cards to add to pile.');
  const deck = game.cards.getName('DoD - lista carte');
  if (!deck)
    return ui.notifications.error('The deck of cards is not available. Please make sure the deck is loaded.');
  const pile = game.cards.getName('Mazzo');
  if (!pile)
    return ui.notifications.error('The pile of cards is not available. Please make sure the pile is loaded.');
  const cards = getSheetCardsToAdd(deck, data);
  if (!cards.length) return ui.notifications.warn('No available cards to add to pile.');
  await deck.pass(pile, cards.map(c => c.id), { chatNotification: false });
...
export async function drawCardsFromPile(sheet) {
  ...
  if (!pile)
    return ui.notifications.error('The pile of cards is not available. Please make sure the pile is loaded.');
  if (!pile.cards.size)
    return ui.notifications.warn('The pile of cards is empty. Please add cards to the pile.');
  const hand = game.cards.getName('Mano');
  if (!hand)
    return ui.notifications.error('The hand of cards is not available. Please make sure the hand is loaded.');
  ...
  if (confirmed) {
    ...
    await hand.draw(pile, Math.ceil(pile.cards.size / (3 + players)), { how: CONST.CARD_DRAW_MODES.RANDOM, chatNotification: false });
    notifyDrawnCardsToChat(hand);
  }
}

function isEmptyCardData(data) {
  return Object.values(data).reduce((sum, card) => sum + Math.max(0, card.value + card.modifier), 0) === 0;
}
function getSheetCardsToAdd(deck, data) {
  const cards = [];
  for (const [cardType, cardObj] of Object.entries(data)) {
    cards.push(...deck.availableCards.filter(c => c.suit === cardType).slice(0, Math.max(0, cardObj.value + cardObj.modifier)));
  }
  return cards;
}
async function passWhiteCardsToPile(deck, pile) {
  const whiteCardsNum = Math.max(0, 20 - pile.cards.size);
  const whiteCards = deck.availableCards.filter(c => c.suit === 'white').slice(0, whiteCardsNum);
  await deck.pass(pile, whiteCards.map(c => c.id), { chatNotification: false });
}
function notifyAddedCardsToChat(pile, data) {
  ChatMessage.create({ user: game.user._id, content: `<p>I added to ${pile.link}: </p><ul><li>Success Cards: ${Math.max(0, data.success.value + data.success.modifier)}</li><li>Failure Cards: ${Math.max(0, data.failure.value + data.failure.modifier)}</li><li>Issue Cards: ${Math.max(data.issue.value + data.issue.modifier)}</li><li>Fortune Cards: ${Math.max(data.fortune.value + data.fortune.modifier)}</li><li>Destiny Cards: ${Math.max(data.destiny.value + data.destiny.modifier)}</li></ul>` });
}
function notifyDrawnCardsToChat(hand) {
  const suitToName = (suit) => ({ white:'White Cards', success:'Success Cards', issue:'Issue Cards', destiny:'Destiny Cards', failure:'Failure Cards', fortune:'Fortune Cards' }[suit]);
  const cardsMap = new Map();
  hand.cards.forEach(card => { cardsMap.set(card.suit, (cardsMap.get(card.suit) || 0) + 1); });
  const cardsHtml = hand.cards.map(card => `<img class="card-face" src="${card.img}" alt="${card.name}" title="${card.name}" style="max-width: 90px;margin-right: 5px;margin-bottom: 5px;"/>`).join('');
  const summary = Array.from(cardsMap).map(([suit, num]) => `<li>${suitToName(suit)}: ${num}</li>`).join('');
  ChatMessage.create({ user: game.user._id, content: `<p>I drew ${hand.link}: </p><ul>${summary}</ul><div class="card-draw flexrow">${cardsHtml}</div>` });
}
